...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(a_C, [n_H * n_W, n_C]) 
        #generated image will have same dim when input so same dim in layer l
    a_G_unrolled = tf.reshape(a_G, [n_H * n_W, n_C])
    
    # compute the cost with tensorflow (≈1 line)
        #kept axis argument empty for reduce_sum to compute sum across all dims
    J_content = tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled, a_G_unrolled))) / (4 * n_C * n_H * n_W) 
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(tf.transpose(a_S), [n_C, n_H * n_W])
        #again, dims of the two images should remain the same so don't have to retreive dims for each
    ...
    # Computing the loss (≈1 line)
        #only need to call reduce_sum once; sums over all dimensions w/o axis argument
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GG,GS))) / (4 * (n_C**2) * (n_H * n_W)**2)
